...
if [ -z "$DSS_DB_PATH" ] || [ -z "$DSS_SECRET_KEY" ] || [ -z "$K3S_SERVER_HOST" ] || [ -z "$K3S_SERVER_SSH_KEY" ] || [ -z "$K3S_SERVER_SSH_USERNAME" ] || [ -z "$DSS_MONGO_USER" ] || [ -z "$DSS_MONGO_PASS" ]; then
    echo "Error: Required environment variables not set in ~/.zshrc"
    echo "Please ensure DSS_DB_PATH, DSS_SECRET_KEY, K3S_SERVER_HOST, K3S_SERVER_SSH_KEY, K3S_SERVER_SSH_USERNAME, DSS_MONGO_USER, and DSS_MONGO_PASS are defined."
    exit 1
fi
...
DSS_MONGO_USER="$DSS_MONGO_USER" DSS_MONGO_PASS="$DSS_MONGO_PASS" envsubst < k8s/mongodb.yaml | kubectl apply -f -
